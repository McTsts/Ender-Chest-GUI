...
#
replaceitem block ~ ~ ~ container.0 bone{eval:1,gui:1}
replaceitem block ~ ~ ~ container.1 bone{eval:1,gui:1}
execute as @s[scores={tsGUI_guiVal=1}] run say #1

replaceitem block ~ ~ ~ container.5 bone{eval:2,gui:1}
...
replaceitem block ~ ~ ~ container.26 lava_bucket{eval:3,gui:1}
execute as @s[scores={tsGUI_guiVal=3}] run scoreboard players set @s tsGUI_guiSwitch 1